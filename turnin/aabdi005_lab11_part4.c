 ...
 *	Demo: https://youtu.be/sUO4SjoQ6MI
...
enum States {Keypad};
enum LCDStates {LCD, Display};
unsigned char positionCursor = 0x01;
...
int LCDTick(int state){
	...
	switch(state){
		case LCD:
            LCD_Cursor(positionCursor);
			break;
		case Display:
			LCD_WriteData(input);
            if(positionCursor > 15) positionCursor = 0x01;
            else positionCursor++;
			break;
		default:
			break;
